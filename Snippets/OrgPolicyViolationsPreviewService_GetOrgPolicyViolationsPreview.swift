...
func sample(
  ...
  let response = try await client.getOrgPolicyViolationsPreview(
...
struct SnippetRunner {
  static func main() async throws {
    do {
      let client = try GoogleCloudPolicySimulatorV1.OrgPolicyViolationsPreviewServiceClient()
      try await sample(
        client: client, organizationId: "[placeholder]", locationId: "[placeholder]",
        orgPolicyViolationsPreviewId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
